...
void applicationWillFinishLaunching(id self, SEL _sel, id notification);
void applicationDidFinishLaunching(id self, SEL _sel, id notification);

/**
* Support for NSApplicationDelegate Protocol
*
*/
NSUInteger applicationShouldTerminate(id self, SEL _sel, id sender)
{
...
/**
* Support for NSWindowDelegate Protocol
*
*/
void windowWillClose(id self, SEL _sel, id notification)
{
...
int main (int argc, char *argv[])
{
	...
	class_addMethod(AppDelegate, applicationDidFinishLaunchingSel, (IMP)applicationDidFinishLaunching,  "v@:@");
	class_addMethod(AppDelegate, applicationWillFinishLaunchingSel, (IMP)applicationWillFinishLaunching,  "v@:@");

	id appDelegate = CreateObject(AppDelegate);
	...
	id winDelegate = CreateObject(WindowDelegate);
	SetDelegate(window, winDelegate);
...
void DoEvents() 
{
	...
	if  (DBG) NSLog($("entering runloop: %@"), NSDefaultRunLoopMode);
...
\
			switch(eventType)
			{
				...
				case 27:
				{
					...
					if  (DBG) NSLog($("mouse moved to %f %f"), p.x, p.y);
					break;
				}
				//case NSLeftMouseDown:
				case 1:
					if  (DBG) NSLog($("mouse left key down"));
					break;
				//case NSLeftMouseUp:
				case 2:
					if  (DBG) NSLog($("mouse left key up"));
					break;
				//case NSRightMouseDown:
				case 3:
					if  (DBG) NSLog($("mouse right key down"));
					break;
				//case NSRightMouseUp:
				case 4:
					if  (DBG) NSLog($("mouse right key up"));
					break;
				...
				case 25:
				{
					...
					if  (DBG) NSLog($("mouse other key down : %i"), (int)number);
					break;
				}
				//case NSOtherMouseUp:
				case 26:
				{
					//NSInteger number = [event buttonNumber];
					NSInteger number = ButtonNumber(event);
					if  (DBG) NSLog($("mouse other key up : %i"), (int)number);
					break;
				}
				//case NSScrollWheel:
				case 22:
				...
				case 12:
				{
					...
					if  (DBG) NSLog($("mod keys : mask %03u state %u%u%u%u%u%u%u%u"), keys.mask, keys.alpha_shift, keys.shift, keys.control, keys.alternate, keys.command, keys.numeric_pad, keys.help, keys.function);
					break;
				}
				//case NSKeyDown:
				case 10:
				{
					...
					if  (DBG) NSLog($("key down %u, text '%s'"), keyCode, inputTextUTF8);
					break;
				}
				//case NSKeyUp:
				case 11:
